...
<div id="header_wrapper" class="header-md" {% if configuration("general","background") %} style="background-image:url({{asset('photos/' ~ configuration("general","background"))}})!important;" {% endif %}>
    <div class="container">
        ...
        <div class="col-xs-12">
            <header id="header">
            <h1>Nueva <?=ucfirst($singularName); ?></h1>
            <ol class="breadcrumb">
...
<div class="row">
    ...
    <div class="card">
        ...
        <h2 class="card-title">Crear <?=ucfirst($singularName); ?></h2>
        <small></small>
        ...
        <div class="card-body">
           {{ form_start(form, {'action': path('<?= $route_name; ?>_new'), 'method': 'post', 'attr': {'class': 'new_form', 'autocomplete': 'off', 'enctype':'multipart/form-data'}}) }}
            <div class="col-md-12">
            ...
            <div class="card-footer m-t-40 m-b-10 border-top">
                <div class="pull-right mt-sm">
                    <input id="submit" class="btn btn-green" type="submit" value="Crear <?=ucfirst($singularName); ?>" />
                </div>
            </div>
            {{ form_end(form) }}
        </div>
